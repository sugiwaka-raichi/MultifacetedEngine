...
char* pbuffer;
string DefaultDirectory = "./Asset/Sound/" ;		//デフォルトディレクトリパス
...
HRESULT InitSound() {
	...
	if (FAILED(hr)) {
		ErrorMessage::ErrorMessageBox("Comコンポーネントの初期化に失敗しました。", "XAudio2 InitError", MB_OK);
	}

	//XAudio2の初期化
	hr = XAudio2Create(&g_pXAudio2);		//XAudio2のインスタンスを返す
	if (FAILED(hr)) {
		ErrorMessage::ErrorMessageBox("XAudio2の作成に失敗しました。", "XAudio2 InitError", MB_OK);
	}

	//マスターボイスは再生されるボイスすべての最終到達地点
	hr = g_pXAudio2->CreateMasteringVoice(&g_pMasteringVoice);		//マスターボイス作成
	if (FAILED(hr)) {
		ErrorMessage::ErrorMessageBox("マスターボイスの作成に失敗しました。", "XAudio2 InitError", MB_OK);
	}
...
void LoadWave(string path, string filename, bool loop) {
	...
	if (finum < 0) {
		string str = "ファイル名:" + filename + "の参照に失敗しました。\nファイルが見つかりませんでした。";
...
void Normalization(DIRECTORY_DATA* _data) {
	...
	int num = FileOperation::GetFileSum(_data, ".wav");
	vector<string> path;
...
void SetSoundType(string oldname, string newname) {
	...
	//見つからなければエラーメッセージを表示
	ErrorMessage::ErrorMessageBox("変更先が見つかりませんでした", "XAudio2 Set Error", MB_OK);
}